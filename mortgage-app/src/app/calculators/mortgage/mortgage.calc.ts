...
export function mortgagePmt(principal: number, annualRate: number, months: number): number {
...
function groupEarlyRepaymentsByMonth(erList: EarlyRepayment[]): Map<number, number> {
...
export function buildMortgageSchedule(
  ...
  const n128Annual = 0.12;

  const today = new Date();
  let balance = loanAmount;
  const rows: AmortizationRow[] = [];
  const erByMonth = groupEarlyRepaymentsByMonth(erList);

  const amortMonths = totalMonths - graceMonths;
  let pmtFixed = mortgagePmt(
    loanAmount,
    fixedRate + n128Annual,
    amortMonths > 0 ? amortMonths : totalMonths,
  );
  let pmtVar = 0;
  ...
  if (fixedMonths > 0 && fixedMonths < totalMonths) {
    ...
    naturalPmtVar = mortgagePmt(natBal, varRate + n128Annual, totalMonths - fixedMonths);
  }
  ...
  for (let m = 1; m <= totalMonths; m++) {
    ...
    const payDate = addMonths(today, m);
    ...
    if (!isFixed && !varPhaseStarted) {
      if (erMode === 'reduceDur' && naturalPmtVar > 0) {
        ...
        pmtVar = mortgagePmt(balance, varRate + n128Annual, remMonths > 0 ? remMonths : 1);
      }
    ...
    if (erAmount > 0) {
      ...
      if (erMode === 'reducePmt' && remMonths > 0 && afterBalance > 0.01) {
        if (isFixed) {
          pmtFixed = mortgagePmt(afterBalance, fixedRate + n128Annual, remMonths);
        } else {
          pmtVar = mortgagePmt(afterBalance, varRate + n128Annual, remMonths);
        }
      }
    }
...
export function computeMortgageSummary(
  ...
  if (!schedule.length) {
    return {
      ...
      grandTotal: 0,
      actualMonths: 0,
      varRate: params.euribor + params.bankMargin,
    };
  }
...
export function computeMortgageErMonthsSaved(
  params: LoanParams,
  erList: EarlyRepayment[],
): ErMonthsSavedMap {
  ...
  for (let i = 0; i < sorted.length; i++) {
    const prevLen = buildMortgageSchedule(params, sorted.slice(0, i)).length;
    const withLen = buildMortgageSchedule(params, sorted.slice(0, i + 1)).length;
