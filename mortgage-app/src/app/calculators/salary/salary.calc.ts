...
const salaryEngine = new SalaryCalculatorEngine();

export function calculateSalary(params: SalaryParams): SalaryResult {
  return salaryEngine.calculate(params);
}

export function buildSalaryParams(
  form: SalaryFormSlice,
  extras?: { annualBonus?: number },
): SalaryParams {
  return salaryEngine.buildSalaryParams(form, extras);
}

export function reverseCalculateSalary(netTarget: number, params: SalaryParams): number {
  return salaryEngine.reverseCalculate(netTarget, params);
}

export function calculateMultiEmployerSalary(params: MultiEmployerParams): MultiEmployerResult {
  return salaryEngine.calculateMultiEmployer(params);
}

export function calculateWithPartialBonusesSalary(params: PartialBonusParams): HolidayBonusResult {
  return salaryEngine.calculateWithPartialBonuses(params);
}

export function calculateTaxOnlySalary(
  taxableIncome: number,
  year: number,
  ageGroup: AgeGroup,
  children: number,
): ReturnType<SalaryCalculatorEngine['calculateTaxOnly']> {
  return salaryEngine.calculateTaxOnly(taxableIncome, year, ageGroup, children);
}
