...
export function calculateInheritanceGift(params: InheritanceGiftParams): InheritanceGiftResult {
  const notes: string[] = [];
  ...
  if (params.transferType === 'monetary') {
    ...
    const result = calcProgressive(taxableBase, INHERITANCE_BRACKETS[params.category]);
    taxDue = result.tax;
