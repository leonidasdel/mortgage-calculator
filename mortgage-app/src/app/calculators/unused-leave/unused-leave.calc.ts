...
function calcTax(
  ...
  const result = calculateTaxOnlySalary(taxable, year, ageGroup, children);
  return {
...
export function calculateUnusedLeave(params: UnusedLeaveParams): LeaveResult {
  ...
  const taxOnTotal = calcTax(totalAnnualTaxable, year, ageGroup, children);
