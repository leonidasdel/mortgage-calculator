...
export class ExportService {
  ...
  printElement(elementId: string): void {
    ...
    if (!w) return;
    w.document.write(`<!DOCTYPE html><html lang="el"><head><meta charset="utf-8"><title>Salaries.gr</title>
      <style>
  ...
  exportPayslipPdf(lines: PayslipLine[], title: string): void {
    ...
    const w = window.open('', '_blank');
    if (!w) return;
    w.document.write(`<!DOCTYPE html><html lang="el"><head><meta charset="utf-8"><title>${title}</title>
  ...
  showToast(msg: string): void {
    ...
    if (!el) {
      ...
      el.id = 'export-toast';
      el.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#1e293b;color:#fff;padding:10px 20px;border-radius:8px;font-size:13px;z-index:9999;box-shadow:0 4px 12px rgba(0,0,0,.3)';
